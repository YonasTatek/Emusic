...
import com.yonas.emusic.databinding.FragmentNowPlayingBinding
...
import com.yonas.emusic.models.QueueData
...
import com.yonas.emusic.ui.dialogs.AboutDialog
...
class NowPlayingFragment : BaseNowPlayingFragment() {
    ...
    private fun setNextData() {
        ...
        if (queue.isNotEmpty() && nowPlayingViewModel.currentData.value != null) {
            ...
            val currentIndex =
                queue.indexOf(nowPlayingViewModel.currentData.value!!.mediaId!!.toLong())
            if (currentIndex + 1 < queue.size) {
                val nextSong = songsRepository.getSongForId(queue[currentIndex + 1])
                setLastFmAlbumImage(
                    binding.upNextAlbumArt,
                    nextSong.artist,
                    nextSong.album,
                    ArtworkSize.MEDIUM,
                    nextSong.albumId
